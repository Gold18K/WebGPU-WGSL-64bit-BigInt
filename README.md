...
Follows that the source must be extremely more verbose than usual, making it unpleasantly long.
So, I decided to split it so that you can choose the best fit for your shader (If you only need 64bit support, there's no need to include the full 2^16 bits source code, that has a total length of around 4500 rows, and just stick with the 64bit one that has around 680).
